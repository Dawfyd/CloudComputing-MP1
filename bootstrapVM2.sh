...
echo "lxd init --auto"
lxd init --auto
echo "lxc launch ubuntu:20.04 LXD2"
lxc launch ubuntu:20.04 LXD2

# APACHE 
echo -e "-- Installing Apache web server\n"
lxc exec LXD2 -- apt-get install -y apache2 

echo -e "-- lxc file push\n"
lxc file push /vagrant/index.html web/var/www/index.html
echo -e "-- Restarting Apache web server\n"
lxc exec LXD2 -- systemctl restart apache2
echo -e "-- lxc config device\n"
lxc config device add LXD2 myport80 proxy listen=tcp:0.0.0.0:80 connect=tcp:127.0.0.1:80
